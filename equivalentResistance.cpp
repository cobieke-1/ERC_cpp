...
struct breadboard{
    ...
    */
     ...
     int lineCounter = 0;
     void addEdge(string name, vector<int> cornerNodes){
        if(!edges.count(name)){
            edges[name] = cornerNodes;
        }
     };
    ...
    // 01/07/23 : MMDDYY , This function was designed after I had started implementing the user interface. I am doing this to make it easier to transfer user input into my graph datastructure.
    void insertComponent(string componentName, int node1, int node2){
        if (node1 < nodes.size())
        {
            nodes[node1].addNode(componentName);
            }
            else{
                // addNode1
                list newEdgeNode1 = list();
            ...
            if(node2 < nodes.size()){
                nodes[node2].addNode(componentName);
            }
            else{
                // addNode2
                list newEdgeNode2 = list();
                newEdgeNode2.addNode(componentName);
                setNewNode(newEdgeNode2);
            }
            vector<int> edgeNodes = {node1, node2}; // creating nodes for the edges/components that is being added
            addEdge(componentName,edgeNodes);
            assignValue(componentName, 0);
     }

     void buildBreadBoard(vector<string> componentNames, unordered_map<string,vector<int>> componentList){
        for(auto name : componentNames){
                insertComponent(name, componentList[name][0], componentList[name][1]);
                if(componentList[name].size() > 2){
                    assignValue(name, componentList[name][2]);
                }
    ...
    */
    ...
    void getCycleRemainder(vector<int> *cycle1, vector<int> *cycle2){ // This function is suppose to "reduce" any existing superset-subset relation into two independent sets. (This function does not take into account hidden superloops)
     ...
     void printOut(){
        ...
        if(!hasCycle()){
         ...
         cout << "Now attempting to calculate the number of cycles in circuit : " << cyclenumber << " cycle(s)" << endl;
        ...
        //  cout << "We want to extract the edges but before that we need to extract the ";
         ...
         displayEdges();
            ...
            */
            ...
            int adjacencyCounter[cycles.size()] ={};
            for (int i = 0; i < cycles.size();i++)
            {
                auto cycle = cycles[i];
                adjacencyCounter[i] = adjacentLookUp[getKeyForCycle(cycle)].size();
                cout << "listing for : ";
                printArray(cycle);
                cout << endl;
                cout << endl;
          ...
          for(auto matrix : loopEquations){
                if(matrix.type() == typeid(vector<int>)) // detecting type of <vector<int>>
                    {
                        cout << "Voltage Source Column : " << endl;
                        voltage = any_cast<vector<int>>(matrix);
                        printArrayVertical(voltage); 
                    }
                   
                else
                    {
                        cout << "Resitance Matrix Column : " << endl;
                        Rmatrix = any_cast<vector<vector<int>>>(matrix);
          ...
          cout << " Now we need the inverse of the Resistance Matrix: " << endl;
          vector<vector<float>> Rinv;
          for (int i = 0; i < Rmatrix.size(); i++){
                vector<float> row(Rmatrix[0].size(), 0);
                Rinv.push_back(row);
          }        
            cout << "\nThe Inverse is :\n";
            if (inverse(Rmatrix, Rinv))
                display(Rinv);

            cout << "Now that we have the inverse, we can multiply the matrix by the voltage vector to get the current vector" << endl;

            cout << "Resulting current Matrix is: ";
            vector<float> currentVector = mulMat(Rinv, voltage);

            cout << " Now we need to get the cycle number that contains the load \"X1\" and divide the resistance along the edge of the load by the corresponding current in the current vector " << endl;
            //for now I will just redundantly divide the voltage by the given matrix.
         
            int index = 0;
            float Requivalent = voltage[index] / currentVector[index];
            cout << Requivalent << " Ohms" << endl;


            /*
                Testing isJustResistors function - returns true if there are only resistors detected in all cycles
            */

            if(isJustResistors(cycles)){ // NEW ADD
                cout << "The loops you detected only have resistors" << endl;
            }
            else{
                cout << "You have loops that are diverse circuit... not just resistors" << endl;
            }

            /*
                Testing getEquivalentResistance function - currently only works with resistor only circuits.
            */

            getEquivalentResistance(cycles, "X1");
        }
        /*
        --------------------------------------------------------------------------
        ...
        */
        ...
        vector<any> getLoopEquations(vector<vector<int>> givenCycles){
            ...
            */
            ...
            for(int i = 0; i < givenCycles.size(); i++){
                ...
                //Add the adjacent loop properties to the loop equation (Note no such thing as a voltage contribution from another loop)
        ...
        void setCycleAdjacents(vector<vector<int>> givenCycles,unordered_map <string, vector<vector<int>>> &adjacentCycleLookUp ){
            ...
            */
            ...
            int cwptr = 1;                            // compare with pointer
            while(cwptr < cycles.size()){
                vector<int> commonEdgeNodes = getCommonNodes(cycles[ctptr], cycles[cwptr]);
               
                if(commonEdgeNodes.size() > 1){         // that means we have atleast an edge (we need 2 nodes to make an edge)
                    adjacentCycleLookUp[getKeyForCycle(cycles[ctptr])].push_back(cycles[cwptr]);
                    adjacentCycleLookUp[getKeyForCycle(cycles[cwptr])].push_back(cycles[ctptr]);
                }
                cwptr++;
                if(cwptr >= cycles.size() && ctptr < cycles.size()-2){
                    ctptr++;
                    cwptr = ctptr + 1;
                }
            }
        ...
        vector<int> singleCycleSum(vector<int> givenCycle){
            ...
            */
            ...
            for(auto pair : cyclePairs){
                ...
                bool load = indicator == 'X';
                int componentValue = edgesValue[componentName];
                if(resistor){
                    resistanceSum += componentValue;
                }
                else if (source || load){ /* Currently : 11/01/23, I am testing equivalent resistance with only resistor circuits. The load would have a voltage of 1v in this scenario (this would be set byt the "getEquivalentResistance" function)*/
                   /*
                   checking polarity:
                   pair: {0,3}
        ...
        /*
        ---------------------------------------------------------------------------------------------
            Matrix Multiplication Functions : from geeksforgeeks (adapted)
        ---------------------------------------------------------------------------------------------
        */

       vector<float> mulMat(vector<vector<float>> Rmatinverse, vector<int> V)
        {
            /*
            This function will retun only a single vector since for my application we won't phave a voltage vector with 2 dimensions. Hence the resulting current vector would always be 1 dimensional.
            */
            int R1 = Rmatinverse[0].size();
            int C2 = 1; // only one column in the vector matrix. 
            int R2 = V.size();

            vector<float> results;
            vector<vector<float>> rslt;
            for (int i = 0; i < R1; i++){
             vector<float> row(C2, 0);
             rslt.push_back(row);
            }
            ...
            for (int i = 0; i < R1; i++) {
                for (int j = 0; j < C2; j++) {
                    ...
                    for (int k = 0; k < R2; k++) {
                        rslt[i][j] += Rmatinverse[i][k] * (V[k]);
                    }
                    results.push_back(rslt[i][j]);
    ...
    /*
     ------------------------------------------------------------------------------------------------
     Inverse of a Matrix : from geeksforgeeks (adapted)
     ------------------------------------------------------------------------------------------------
    */
                // C++ program to find adjoint and inverse of a matrix - I just did this in javascript
        ...
        // matrix is singular
        bool inverse(vector<vector<int>> A, vector<vector<float>> &inverse)
        {
        ...
        /*
        --------------------------------------------------------------------------------------------------------------------------------
        MATRIX functions with float parameters.
        --------------------------------------------------------------------------------------------------------------------------------
        */

        void getCofactor(vector<vector<float>> A, vector<vector<float>> &temp, int p, int q,
                        int n)
        {
            int i = 0, j = 0;
        ...
        /*
        Function to check if all the components (valid cycles) are just resistors
            We do not count any lines added or the load itselft. Note we are using the
            thevenin approach to calculate equivalent resistance.
        */

        bool isJustResistors(vector<vector<int>> givencycles){ // NEW ADD
            for(auto cycle : givencycles){
                auto cycleEdges = getPairedNodes(cycle);
                for(auto pair : cycleEdges){
                    string componentName = getPairEdge(pair[0], pair[1]);
                    cout << componentName << componentName[0] << componentName[1] << endl;
                    if((componentName[0] != 'R' ) && (componentName[0] != 'L') && (componentName[0] != 'X')){
                        cout << (componentName[0] == 'L') << endl;
                        return false;
                    }
                }
            }
     ...
     float getEquivalentResistance(vector<vector<int>> givencycles, string loadName){ // loadName : "X1". However I can potentially imagine users having multiple circuits with different loads on the same breadboard.
            if(isJustResistors(givencycles)){
                vector<int> loopNum;
                vector<vector<int>> loopsWithX = getBorderLoopsOf(edges[loadName], givencycles, loopNum);
                assignValue(loadName, 1);
                
                
                cout << "These are the border loops of the load : " << loadName << endl;
                for(auto elem : loopsWithX){
                    printArray(elem);
                }
                cout << endl;

                // now get loop matricies.
                vector<any> loopEquations = getLoopEquations(givencycles);
                vector<int> voltage = any_cast<vector<int>>(loopEquations[1]);
                vector<vector<int>> Rmatrix = any_cast<vector<vector<int>>>(loopEquations[0]);
                 vector<vector<float>> Rinv;
                for (int i = 0; i < Rmatrix.size(); i++){
                    vector<float> row(Rmatrix[0].size(), 0);
                    Rinv.push_back(row);
                }
                inverse(Rmatrix, Rinv);
                

                vector<float> current = mulMat(Rinv, voltage);
                cout << "This is the current ( in Amperes) for each loop" << endl;
                printArray(current);
                cout << endl;
                //get associated  equation number/ loop number.
                float loadCurrent = 0;
            
                for(auto eachLoop : loopNum){
                    loadCurrent += current[eachLoop]*getPolarityWithCycle(givencycles[eachLoop], loadName);
                }
               
                float equivResistance =  abs(edgesValue[loadName] / loadCurrent);
                cout <<" The equivalent resistance of this circuit is : "<< equivResistance << " Ohms" << endl;
            }
            else{
                // TODO: here we have to include voltage sources, current sources, dependent sources etc. To be determined

            }
            return 1;
     }

     vector<vector<int>> getBorderLoopsOf(vector<int> edgeX,vector<vector<int>> givencycles, vector<int> &loopNumber){
            vector<vector<int>> borderLoops;
            // vector<int> loopNumber; // the loop equation number of the bordering cycles.

            for (int i = 0; i < givencycles.size(); i++){
                auto currCycle = givencycles[i];
                printArray(currCycle);
                bool cycleIsBorder = true;
                int cwNodePtr = 0; // points to the node that we are currently checking at edgeX
                while(cwNodePtr < edgeX.size()){
                    if (!checkElemExist(edgeX[cwNodePtr], currCycle)){
                        //skip this cycle
                        cout << "Cycle not a border-cycle" << endl;
                        cycleIsBorder = false;
                        break;
                    }else{
                        cwNodePtr++;
                ...
                if(cycleIsBorder){
                    cout << "Cycle is a border-cycle" << endl;
                    loopNumber.push_back(i);
                    borderLoops.push_back(currCycle);
                }
                if(loopNumber.size() == 2){
                    break; // we know there can only be 2 border loops per edge.
                }
            }
            return borderLoops;
     }
...
*/
    ...
    // hw1.insertComponent("Ve",0,1);
    // hw1.assignValue("Ve", 24);
    // hw1.insertComponent("R1", 1,2);
    // hw1.assignValue("R1", 200);
    // hw1.insertComponent("R2", 2, 0);
    // hw1.assignValue("R2", 200);
    // hw1.insertComponent("R3", 2,3);
    // hw1.assignValue("R3", 50);
    // hw1.insertComponent("R4", 3,4);
    // hw1.assignValue("R4", 150);
    // hw1.insertComponent("L1", 0, 4);

    // // New insert method - now you add components directly to their nodes, and build them with the "buildBreadBoard" function
    ...
    //     "L8"
    // };
    // unordered_map<string, vector<int>> componentList;
    // componentList["R0"] = {0, 1, 24};
    // componentList["R1"] = {1, 2, 200};
    // componentList["R2"] = {2, 0, 200};
    // componentList["R3"] = {2, 3, 50};
    // componentList["R4"] = {3, 4, 150};
    // componentList["R5"] = {0, 4, 10};
    // componentList["R6"] = {4, 5, 10};
    // componentList["X1"] = {5, 6, 0};
    // componentList["L8"] = {0, 6};
    ...
    // };
    // unordered_map<string, vector<int>> componentList;
    // componentList["R1"] = {0, 1, 200};
    // componentList["R2"] = {2, 3, 200};
    // componentList["L1"] = {1, 3, 0};
    // componentList["L2"] = {0, 2, 0};
    // componentList["R3"] = {3, 4, 50};
    // componentList["R4"] = {4, 2, 150};
    // componentList["L3"] = {4, 5, 0};
    // componentList["L4"] = {2, 6, 0};
    // componentList["X1"] = {5, 6, 0};
    ...
    // };
    // unordered_map<string, vector<int>> componentList;
    // componentList["X1"] = {0, 1, 0};
    // componentList["R1"] = {1, 2, 10};
    // componentList["R2"] = {2, 0, 3};
    // componentList["R3"] = {2, 3, 6};
    // componentList["R4"] = {2, 4, 1};
    // componentList["R5"] = {0, 4, 12};
    // componentList["R6"] = {4, 3, 4};
    // componentList["R7"] = {4, 5, 1};
    // componentList["R8"] = {5, 6, 5};
    // componentList["L1"] = {0, 3, 0};
    // componentList["L2"] = {3, 6, 0};
    ...
    componentList["R1"] = {0, 1, 2};
    componentList["L1"] = {1, 2, 0};
    componentList["L2"] = {2, 3, 0};
    componentList["R2"] = {0, 3, 1};
    componentList["R3"] = {3, 4, 1};
    componentList["L3"] = {4, 5, 0};
    componentList["R4"] = {0, 5, 2};
    componentList["L4"] = {5, 6, 0};
    componentList["L5"] = {1, 7, 0};
    componentList["X1"] = {7, 6, 0};
    
    
    

    hw1.buildBreadBoard(componentNames, componentList);

     /*
    ---------------------------------------------------------------------------------------------------
    Circuit for testing equivalent resistance
    ---------------------------------------------------------------------------------------------------
    */
    //series connection
    // vector<string> componentNames = {
    ...
    // unordered_map<string, vector<int>> componentList;
    // componentList["R1"] = {0, 1, 24};
    // componentList["R2"] = {1, 2, 200};
    // componentList["R3"] = {2, 3, 50};
    // componentList["R4"] = {3, 4, 150};
    // componentList["V1"] = {0, 4, 1};
